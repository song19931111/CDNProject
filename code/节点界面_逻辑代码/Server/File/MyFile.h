...
////////////////////////////////////////////////////////////////////////////////
	...
	public :
		char *pMd5;//存放md5 信息
		MyLock m_lock_read_md5;
		char *ReadMd5(int nBlockIndex,char arrMd5[],int Length)
		{
			if(Length !=33 || arrMd5 == NULL)
			{
				return NULL;
			}
			if (nBlockIndex *33 >=m_nProfileSize)
			{
				return NULL;
			}
			m_lock_read_md5.Lock();
			FILE *fp ;
			fopen_s(&fp,m_chAllPath,"rb");
			fseek(fp,nBlockIndex*33,SEEK_SET);
			int nRet  =fread(arrMd5,33,1,fp);
			fflush(fp);
			fclose(fp);
			m_lock_read_md5.UnLock();
			if(-1 ==nRet)
			{
				return NULL;
			}
